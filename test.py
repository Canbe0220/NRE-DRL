...
def main():
    ...
    mod_files = os.listdir('./test_model/')[:]

    memories = DRL.Memory()
    model = DRL.PPO(model_paras, train_paras)
    rules = test_paras["rules"]
    envs = []  # Store multiple environments

    # Detect and add models to "rules"
    if "DRL" in rules:
        for root, ds, fs in os.walk('./test_model/'):
            for f in fs:
                if f.endswith('.pt'):
                    rules.append(f)
    if len(rules) != 1:
    ...
    for i_rules in range(len(rules)):
        ...
        if rule.endswith('.pt'):
            if device.type == 'cuda':
                model_CKPT = torch.load('./test_model/' + mod_files[i_rules])
            else:
                model_CKPT = torch.load('./test_model/' + mod_files[i_rules], map_location='cpu')
        ...
        for i_ins in range(num_ins):
            ...
            with open(test_file) as file_object:
                ...
                ins_num_jobs, ins_num_mas, ins_num_fac, _ = nums_detec(line)
            env_test_paras["num_jobs"] = ins_num_jobs
            env_test_paras["num_mas"] = ins_num_mas
            env_test_paras["num_fac"] = ins_num_fac

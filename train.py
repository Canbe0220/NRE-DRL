...
def main():
    ...
    num_fac = env_paras["num_fac"]

    opes_per_job_min = int(num_mas * 0.8)
    opes_per_job_max = int(num_mas * 1.2)

    memories = DRL.Memory()
    model = DRL.PPO(model_paras, train_paras, num_envs=env_paras["batch_size"])
    ...
    for i in range(1, train_paras["max_iterations"]+1):
        ...
        if (i - 1) % train_paras["parallel_iter"] == 0:
            ...
            case = CaseGenerator(num_jobs, num_mas, num_fac, opes_per_job_min, opes_per_job_max, nums_ope=nums_ope)
            env = gym.make('fjsp-v0', case=case, env_paras=env_paras)
            print('num_job: ', num_jobs, '\tnum_mas: ', num_mas, '\tnum_fac: ', num_fac, '\tnum_opes: ', sum(nums_ope))
